...
qboolean ACEMV_SpecialMove(edict_t *self, usercmd_t *ucmd)
{
	...
	if(tr.allsolid)
	{
		...
		// Check for jump
		start[2] += 32; //hypov8 48? what are we checking for
		end[2] += 32; //hypov8 max height we can jump is 60
		tr = gi.trace (start, self->mins, self->maxs, end, self, MASK_MONSTERSOLID);
...
///////////////////////////////////////////////////////////////////////
qboolean ACEMV_MoveToGoal(edict_t *self, usercmd_t *ucmd)
{
...
// acebot //hypo change grenade to move back. disable walk forward to?
	else if (strcmp(self->movetarget->classname, "grenade") == 0)
	{
		...
		// strafe left/right
		if (rand() % 1 && ACEMV_CanMove(self, MOVE_BACK))
		{
			ucmd->forwardmove= -400;
			ucmd->sidemove = 0;
		}
