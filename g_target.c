...
int timerValue[10][MAX_CLIENTS];	// stores frame-# of all timers
...
void target_timer_use (edict_t *self, edict_t *other, edict_t *activator)
{
    ...
    // activated by player?
    if (TeamID>=0 && TeamID<10 && activator && activator->client)
    {
	    // init
		PlayerID = (activator - g_edicts - 1);
...
void SP_target_timer (edict_t *self)
{
    ...
    if (!self->team)
        gi.dprintf("timer: no team %s at %s\n", self->classname, vtos(self->s.origin));

    if (!self->targetname)
        gi.dprintf("timer: no targetname %s at %s\n", self->classname, vtos(self->s.origin));

	{
	    int TeamID;
		TeamID = atoi(self->team);
	    if (TeamID>=0 && TeamID<10)
			memset(timerValue[TeamID],MAX_CLIENTS*sizeof(int),0);
		else
			gi.dprintf("timer: id %s out of range at %s\n", self->team, vtos(self->s.origin));
	}
