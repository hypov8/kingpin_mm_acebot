...
void HookBehavior(edict_t *ent) {
  ...
  if ((!(ent->owner->client->hookstate & HOOK_ON)) ||
	...
	// BEGIN HOOK - have we been swinging for long enough?
	if (level.time >= ent->owner->client->hook_attach_time)
		{
	    DropHook(ent);
	    return;
		}
...
void HookTouch (edict_t *ent, edict_t *other, cplane_t *plane, 
  ...
  ent->owner->client->hookstate |= HOOK_IN;
	...
	// BEGIN HITMEN
	ent->owner->client->hook_attach_time = level.time + hook_hold_time->value;
	// END
